...
import { ChevronLeft, ChevronRight, ExclamationTriangle } from 'react-bootstrap-icons';
...
const EditSimulation = () => {
  ...
  const { _id } = useParams();
  const { simulation, models, ready } = useTracker(() => {
  ...
  const saveSimulation = () => {
    ...
    Simulations.collection.update(
      _id,
      { $set: { base64: base64, cost: totalCost, models: modelsToUpload } },
      (error) => {
        if (error) {
          swal('Error', error.message, 'error');
        } else {
          swal('Success', 'Your simulation has been updated!', 'success');
  ...
  const deleteSimulation = () => {
    Simulations.collection.remove({ _id: _id });
  ...
  useEffect(() => {
            ...
            loader.load(
              ...
              (gltf) => {
                ...
                gltfscene.userData = {
                  ...
                  coords: { lng: model.coords.lng, lat: model.coords.lat },
                  position: { x: gltfscene.position.x, y: gltfscene.position.y, z: gltfscene.position.z },
                  rotation: { x: gltfscene.rotation.x, y: gltfscene.rotation.y, z: gltfscene.rotation.z },
                  scale: { x: gltfscene.scale.x, y: gltfscene.scale.y, z: gltfscene.scale.z },
                  cost: (model.cost !== undefined) ? model.cost : 0,
                };
                scene.add(gltfscene);
              },
            );
    ...
    jumpToMap({ lng: simulation.mapCenter.lng, lat: simulation.mapCenter.lat });
  });
  ...
  const filterModels = () => {
    const availableModels = _.filter(models, (model) => model.show);
    return _.filter(availableModels, (model) => {
      if (searchField === '') {
    ...
    <div>
        ...
        <Col lg={3} className="pt-3" id="edit-window" style={{ paddingRight: 20 }}>
          ...
          <Row>
            ...
            <div id="edit-none">
              ...
              <p>When updating the simulation, the current camera angle you see will be used as the starting view upon loading the simulation.</p>
              <Button variant="success" onClick={saveSimulation}>Update Simulation</Button>
              <Spinner id="loading-spinner" animation="border" variant="info" size="lg" style={{ position: 'relative', left: '1rem', bottom: '-0.5rem' }} hidden />
              <br />
              <Button variant="danger" onClick={() => setShowDeleteModal(true)}>Delete Simulation</Button>
            </div>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} style={{ marginTop: '20vh' }}>
        <Modal.Body className="py-5" style={{ textAlign: 'center' }}>
          ...
          <h2>Are you sure?</h2>
          <p><b>You are about to delete this simulation.</b></p>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowDeleteModal(false)}>
            Cancel
          </Button>
          <Button variant="danger" onClick={deleteSimulation}>
            Delete
          </Button>
        </Modal.Footer>
      </Modal>
      <Modal scrollable show={showModel} onHide={handleClose} size="xl">
